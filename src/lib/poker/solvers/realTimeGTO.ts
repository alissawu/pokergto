 ...
 * We limit depth and abstract cards to make it tractable
 */
export class RealTimeGameTree {
  ...
  private buildNode(
    ...
    const infoSet = this.createInfoSet(state, currentPlayer === 0 ? heroId : villain?.id || '', history);
    
    // Get legal actions (simplified)
    const actions = this.getSimplifiedActions(state, currentPlayer === 0 ? hero : villain!);
    const children = new Map<Action, TreeNode>();
    
    // Build child nodes for each action
    for (const action of actions.slice(0, maxActions)) {
      const nextState = this.simulateAction(state, action, currentPlayer === 0 ? hero : villain!);
      const nextHistory = history + action[0].toUpperCase(); // F, C, R, B for fold/call/raise/bet
      const nextPlayer = 1 - currentPlayer;
      
      children.set(
        action,
        this.buildNode(nextState, heroId, nextPlayer, depthRemaining - 1, nextHistory, maxActions)
      );
    }
  ...
  private createTerminalNode(state: GameState, heroId: string, history: string): TreeNode {
    const hero = state.players.find(p => p.id === heroId)!;
    const villain = state.players.find(p => p.id !== heroId && !p.hasFolded);
  ...
  private createInfoSet(state: GameState, playerId: string, history: string): string {
  ...
  private simulateAction(state: GameState, action: Action, player: Player): GameState {
    // Clone state
    ...
    switch (action) {
      ...
      case 'call':
        const toCall = Math.min(newState.currentBet - newPlayer.currentBet, newPlayer.stack);
  ...
  private estimateEquity(hero: Player, villain: Player, state: GameState): number {
    ...
    const bucketEquities: Record<string, number> = {
      ...
      'broadway_suited': 0.50,
      'broadway_offsuit': 0.45,
      'good_suited': 0.45,
      'suited_connector': 0.40,
      'decent_suited': 0.38,
      'decent_offsuit': 0.35,
  ...
  private collectInfoSets(node: TreeNode, infoSets: Map<string, CFRNode>): void {
